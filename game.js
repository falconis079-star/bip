...
const G = {
  ...
  room: 0, score: 0, best: Number(localStorage.getItem("bip_best") || 0),
  hp: 3, shield: 0, combo: 0, comboTime: 0,
  cleared: 0, need: 10, spawn: 0, banner: "", bannerT: 0,
...
function hudSync() {
  $("roomHud").textContent = ROOMS[G.room].name;
  $("waveHud").textContent = (G.room + 1) + "/10";
...
function stats(kind, boss) {
  if (boss) return { hp: 6, r: 58, worth: 200, spd: 0.62 };
...
function startRun() {
  ...
  G.combo = 0; G.comboTime = 0; G.cleared = 0; G.need = 8;
  G.ents = []; G.bits = []; G.flashes = [];
  G.taught = false; G.roomHit = false; G.bossOut = false;
  G.banner = "ЖМИ ПО ШАРУ"; G.bannerT = 1.6; G.spawn = 0;
...
function roomClear() {
  ...
  if (clean) G.score += 150;
  const room = ROOMS[G.room];
  const next = ROOMS[G.room + 1];
  $("roomOverTitle").textContent = room.name + " зачищена";
  $("roomOverText").textContent = (clean ? "Бипа не задели. +150. " : "") +
    (next ? "Дальше: " + next.name : "Это был финал.");
  const showClip = CLIP_ROOMS.includes(G.room);
  $("clipBtn").style.display = showClip ? "block" : "none";
...
function goNext() {
  ...
  G.room += 1;
  G.cleared = 0; G.need = 8 + G.room;
  G.ents = []; G.roomHit = false; G.bossOut = false;
  G.banner = ROOMS[G.room].name.toUpperCase(); G.bannerT = 1.1; G.spawn = 0;
  G.running = true; G.between = false;
  $("pauseBtn").classList.remove("hidden");
  hudSync(); beep(660, 0.1, "triangle", 0.05);
}
...
function finish(win) {
  ...
  $("endText").textContent = win ? "Все 10 комнат. Счёт " + G.score : ROOMS[G.room].name + ". Счёт " + G.score;
...
function hitEnt(e) {
  ...
  G.score += e.worth + Math.min(90, G.combo * 8);
...
function tap(x, y) {
  ...
  if (best) { hitEnt(best); G.ents = G.ents.filter(e => e.hp > 0); }
  else burst(x, y, "rgba(255,255,255,.35)", 4);
}
...
function loop(ts) {
  ...
  G.spawn += dt;
  const rate = Math.max(0.42, 1.08 - G.room * 0.05);
  if (!G.bossOut && G.cleared >= G.need) {
    G.bossOut = true; spawn(true); G.banner = "БОСС КОМНАТЫ"; G.bannerT = 1;
  } else if (!G.bossOut && G.spawn > rate && G.ents.length < 7 && G.bannerT < 0.35) {
    G.spawn = 0; spawn(false);
  }
  if (G.bossOut && G.ents.length === 0) roomClear();
...
function drawProp(kind, w, h) {
  ...
  ctx.lineWidth = 3;
  if (kind === "mirror") {
    ctx.strokeRect(w * 0.62, h * 0.18, 70, 150);
    ctx.strokeRect(w * 0.62 - 8, h * 0.16, 86, 170);
  } else if (kind === "vac") {
    ctx.beginPath(); ctx.arc(w * 0.18, h * 0.62, 28, 0, Math.PI * 2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(w * 0.18 + 24, h * 0.6); ctx.quadraticCurveTo(w * 0.4, h * 0.3, w * 0.55, h * 0.34); ctx.stroke();
  } else if (kind === "wash") {
    ctx.strokeRect(w * 0.12, h * 0.42, 90, 110);
    ctx.beginPath(); ctx.arc(w * 0.12 + 45, h * 0.42 + 62, 28, 0, Math.PI * 2); ctx.stroke();
  } else if (kind === "kettle") {
    ctx.beginPath(); ctx.ellipse(w * 0.78, h * 0.48, 28, 22, 0, 0, Math.PI * 2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(w * 0.78 + 28, h * 0.48); ctx.quadraticCurveTo(w * 0.9, h * 0.42, w * 0.78 + 22, h * 0.38); ctx.stroke();
  } else if (kind === "fridge") {
    ctx.strokeRect(w * 0.7, h * 0.2, 70, 200);
    ctx.beginPath(); ctx.moveTo(w * 0.7, h * 0.36); ctx.lineTo(w * 0.7 + 70, h * 0.36); ctx.stroke();
  } else if (kind === "tv") {
    ctx.strokeRect(w * 0.18, h * 0.2, 160, 96);
    ctx.strokeRect(w * 0.18 + 70, h * 0.2 + 96, 20, 24);
  } else if (kind === "box") {
    ctx.strokeRect(w * 0.14, h * 0.5, 90, 70);
    ctx.beginPath(); ctx.moveTo(w * 0.14, h * 0.5); ctx.lineTo(w * 0.14 + 45, h * 0.42); ctx.lineTo(w * 0.14 + 90, h * 0.5); ctx.stroke();
  } else if (kind === "fox") {
    ctx.beginPath(); ctx.moveTo(w * 0.7, h * 0.48); ctx.lineTo(w * 0.76, h * 0.34); ctx.lineTo(w * 0.82, h * 0.48); ctx.closePath(); ctx.stroke();
    ctx.beginPath(); ctx.arc(w * 0.76, h * 0.54, 22, 0, Math.PI * 2); ctx.stroke();
  } else if (kind === "cat") {
    ctx.beginPath(); ctx.arc(w * 0.22, h * 0.5, 22, 0, Math.PI * 2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(w * 0.12, h * 0.42); ctx.lineTo(w * 0.16, h * 0.32); ctx.lineTo(w * 0.2, h * 0.42); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(w * 0.24, h * 0.42); ctx.lineTo(w * 0.28, h * 0.32); ctx.lineTo(w * 0.32, h * 0.42); ctx.stroke();
  } else {
    ctx.strokeRect(w * 0.12, h * 0.22, 50, 70);
    ctx.strokeRect(w * 0.7, h * 0.28, 60, 40);
    ctx.beginPath(); ctx.arc(w * 0.5, h * 0.4, 24, 0, Math.PI * 2); ctx.stroke();
  }
  ctx.restore();
}
...
function drawBip(x, y) {
  ...
  ctx.fillStyle = "#7dffb0"; ctx.beginPath(); ctx.arc(0, -46, 5, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "#9ad7ff"; ctx.fillRect(-2, -42, 4, 10);
  ctx.fillStyle = "#eef7ff";
  ctx.beginPath(); ctx.roundRect(-28, -32, 56, 42, 16); ctx.fill();
  ...
  ctx.beginPath(); ctx.roundRect(-22, 12, 44, 28, 10); ctx.fill();
  ctx.fillStyle = "#08314a"; ctx.font = "900 12px sans-serif"; ctx.textAlign = "center"; ctx.fillText("БИП", 0, 30);
  if (G.shield > 0) {
    ctx.strokeStyle = "rgba(108,255,178,.85)"; ctx.lineWidth = 3;
...
function draw() {
  ...
  for (const f of G.flashes) {
    ctx.globalAlpha = f.life; ctx.strokeStyle = f.color; ctx.lineWidth = 3;
  ...
  if (G.combo > 1) {
    ctx.fillStyle = "#ffd45a"; ctx.font = "900 28px sans-serif"; ctx.textAlign = "center";
    ctx.fillText("КОМБО ×" + G.combo, G.w / 2, G.h * 0.16);
  }
  if (G.bannerT > 0) {
    ctx.globalAlpha = Math.min(1, G.bannerT * 2); ctx.fillStyle = "#fff";
    ctx.font = "900 32px sans-serif"; ctx.textAlign = "center";
    ctx.fillText(G.banner, G.w / 2, G.h * 0.27); ctx.globalAlpha = 1;
  }
...
$("shareBtn").onclick = () => VKGame.openGroup();
...
$("clipBtn").onclick = () => {
  ...
  if (clip && clip.id) {
    ...
    VKGame.openGroup();
    box.innerHTML = "<div style='padding:16px'>Открыл группу Бипа. Потом жми «Дальше в бой».</div>";
  }
};
...
$("menuBtn").onclick = () => {
  ...
  $("pause").classList.add("hidden"); $("hud").classList.add("hidden");
  $("pauseBtn").classList.add("hidden"); $("roomOver").classList.add("hidden");
