...
function syncStats() {
  const base = window.BIP_CONFIG && BIP_CONFIG.statsUrl;
  if (!base) {
    if ($("liveLine")) $("liveLine").textContent = "Счётчик подключится после таблицы";
    return;
  }
  const p = payload();
  const q = Object.keys(p).map(k => encodeURIComponent(k) + "=" + encodeURIComponent(p[k])).join("&");
  fetch(base + (base.indexOf("?") >= 0 ? "&" : "?") + q)
    .then(r => r.json())
    .then(d => {
      if ($("liveLine")) $("liveLine").textContent = "Играют " + (d.players || 0);
    })
    .catch(() => {
      if ($("liveLine")) $("liveLine").textContent = "Счётчик не ответил — проверь URL скрипта";
    });
}
