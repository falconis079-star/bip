...
const STORE = "bip_save_v2";
function loadSave() {
  try { return JSON.parse(localStorage.getItem(STORE) || "{}"); } catch (e) { return {}; }
}
function writeSave(part) {
  const s = Object.assign(loadSave(), part, { best: Math.max(G.best, G.coins) });
...
function refreshMenu() {
  ...
  const s = loadSave();
  $("dailyLine").textContent = s.daily === today()
    ? "Ежедневка уже получена. Серия: " + (s.streak || 1)
    : "Можно забрать ежедневный бонус";
}
...
function startRun() {
  ...
  G.banner = G.room === 0 ? "ЖМИ ПО ШАРУ" : ROOMS[G.room].name.toUpperCase();
...
$("clipMenuBtn").onclick = () => { G.clipPaid = false; openClipFrom("playerBox"); VKGame.openGroup(); };
...
if (daily) $("dailyLine").textContent = "Ежедневка +" + daily + " BipCoin";
refreshMenu();
requestAnimationFrame(loop);
